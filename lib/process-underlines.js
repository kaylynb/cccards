...
const underlineSpacer = (count = 1) =>
	`<span class="underline">${'&nbsp;'.repeat(count)}</span>`
...
module.exports = text => {
	...
	const processUnderline = part => {
		...
		if (matches) {
			...
			return `<div class="underline${underlineLength > 1 ? ' manual' : ''}">${
				matches[1]
			}${underlineSpacer(underlineLength)}${matches[3]}</div>`
		}
	...
	return {
		text: text
			.split(underlinePatternRegex)
			.map(processUnderline)
			.join(''),
