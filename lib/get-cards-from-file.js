...
const loadFromJson = async cardsPath => {
	return JSON.parse(
		await fs.readFile(cardsPath, { encoding: 'utf8' })
	)
}

module.exports = async (cardsPath, title) => {
	const cards = path.extname(cardsPath) === '.xlsx'
		? await loadFromXlsx(cardsPath)
		: await loadFromJson(cardsPath)
